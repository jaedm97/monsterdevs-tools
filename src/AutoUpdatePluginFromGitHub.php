...
class AutoUpdatePluginFromGitHub {
	 ...
	 * @var int
	 */
	private $last_update_check_key = 'monsterdevs_tools_last_update_check';
	...
	public function force_update_check() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' === $pagenow && function_exists( 'wp_clean_plugins_cache' ) && function_exists( 'wp_update_plugins' ) && function_exists( 'get_site_transient' ) ) {
			...
			try {
				 ...
				 */
				if ( ( ( empty( $transient ) || empty( $transient->response[ $this->plugin_slug ] ) || version_compare( $this->current_version, $transient->response[ $this->plugin_slug ]->new_version, '>=' ) ) && ( ( $current_time - $last_check ) > $check_interval ) ) )
				) {
