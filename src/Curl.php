...
class Curl {
	...
	public static function do_curl( $endpoint, $body = array(), $headers = array(), $method = 'POST', $api_version = 'v2', $api_key = '', $api_domain = '' ) {
		...
		if ( empty( $api_url ) ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Invalid or Empty API Domain', 'monsterdevs-tools' ),
			);
		}
		...
		if ( empty( $api_key ) ) {
			return array(
				'success' => false,
				'message' => esc_html__( 'Invalid or Empty API Key', 'monsterdevs-tools' ),
			);
		}
		...
		if ( defined( 'MONSTERDEVS_DEBUG_LOG' ) && MONSTERDEVS_DEBUG_LOG ) {
